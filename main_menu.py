...
def open_orders_menu():
    ...
    subprocess.run(['python3', open_orders_path])
...
def eod_reports_menu():
    ...
    subprocess.run(['python3', eod_reports_path])

# GUI Setup
root = tk.Tk()
root.title("Main Menu")
root.geometry("300x300+100+100")  # Fixed window size (adjust the width and height as needed)

# Add Labels and Buttons for the Options
tk.Label(root, text="Choose a report:", font=("Arial", 14)).pack(pady=20)

# Option Buttons
tk.Button(root, text="Open Orders Reports", command=open_orders_menu, width=15).pack(pady=10, padx=50, anchor="center")
tk.Button(root, text="End of Day Reports", command=eod_reports_menu, width=15).pack(pady=10, padx=50, anchor="center")
